...
async function checkBackorder() {
    const { count, error } = await oldSupabase.from('backordered_items').select('*', { count: 'exact', head: true });
    console.log(`backordered_items: ${count} rows (Error: ${error?.message || 'none'})`);

    if (count && count > 0) {
        const { data } = await oldSupabase.from('backordered_items').select('*').limit(5);
