...
async function checkPrintControl() {
    const { count, error } = await oldSupabase.from('print_control').select('*', { count: 'exact', head: true });
    console.log(`print_control: ${count} rows (Error: ${error?.message || 'none'})`);

    if (count && count > 0) {
        const { data } = await oldSupabase.from('print_control').select('*').limit(1);
