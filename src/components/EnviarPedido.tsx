...
import { parseSku, getEffectiveQuantity, buildSku, getCategory, isKit, transformSku, isPersonalizado } from '../services/skuService';
...
const formatSizeQuantities = (sizes: Record<string, number>): string => {
    ...
    const activeSizes = sortedSizes.filter(size => sizes[size] > 0);
...
const getDisplayColorName = (color: string, store: string) => {
    if (store === 'GLOBAL' && color.toUpperCase() === 'MESCLA') {
        return 'MESCLA/CINZA/CINZA CLARO';
    }
    ...
    Array.from(groupedData.entries())
        ...
        .forEach(([groupName, items]) => {
            ...
            items.forEach((order) => {
                ...
                // Check if order has any items > 0
                const hasAnyItem = sortedColors.some(color => {
                    const sizeStr = formatSizeQuantities(quantities[color]);
                    return sizeStr !== '';
                });
                ...
                // Only show CNPJ if it's NOT a combined one (Ambos, Ambos Unidos, Todos)
                if (order.cnpj !== 'Ambos Unidos' && order.cnpj !== 'Ambos' && order.cnpj !== 'Todos') {
                    productTitle += ` (${order.cnpj})`;
                }
                // Title size 11pt, Arial is enforced by body css
                body += `<h3 style="font-size: 11pt; font-weight: bold; margin-bottom: 3px;">${productTitle}</h3>`;

                // Using div instead of ul/li to avoid bullets in Word
                body += '<div style="margin-bottom: 5px;">';
                sortedColors.forEach(color => {
                    const sizeString = formatSizeQuantities(quantities[color]);
                    if (!sizeString) return; // Skip if no quantity
                    ...
                    // Simple paragraph style for item line
                    const displayColorName = getDisplayColorName(color, store);
                    body += `<div style="margin-bottom: 2px;"><span style="display:inline-block; width:10px; height:10px; border:1px solid #333; margin-right: 6px; vertical-align: middle;"></span><strong style="color: ${fontColor};">${displayColorName}:</strong> ${sizeString}</div>`;
                });
                body += '</div>';

                let totalsHtml = `<p style="font-weight: bold; margin-top: 3px;">TOTAL: ${totals.totalGeral}`;
                if (store === 'GUSHI' && totals.totalEspeciais) {
                    totalsHtml += ` <span style="font-weight: normal;">(Especial: ${totals.totalEspeciais})</span>`;
                }
                totalsHtml += `</p>`;
                body += totalsHtml;

                // Horizontal separator between products (Text based as requested)
                body += `<p style="margin-top: 10px; margin-bottom: 10px; color: #333;">------------------------------------------------------------------------------------------------</p>`;

                body += `</div>`;
            });
        });

    const css = `body{font-family: Arial, sans-serif; font-size: 9pt; line-height: 1.2;}`;
    ...
    const { groupedData, plainTextData } = React.useMemo(() => {
        ...
        sortedGroupedData.forEach(([groupName, items], groupIndex) => {
            ...
            items.forEach((order) => {
                ...
                // Pre-check if there are any valid items
                const validColors = Object.keys(quantities).sort().filter(color => {
                    const sizeString = formatSizeQuantities(quantities[color]);
                    return sizeString !== '';
                });
                ...
                if (groupBy === 'store') {
                    let productTitle = product.toUpperCase();
                    if (order.cnpj !== 'Ambos Unidos' && order.cnpj !== 'Ambos' && order.cnpj !== 'Todos') {
                        productTitle += ` (${order.cnpj})`;
                    }
                ...
                validColors.forEach(color => {
                    const sizeString = formatSizeQuantities(quantities[color]);
                    const displayColorName = getDisplayColorName(color, store);
                    content += `[ ] ${displayColorName}: ${sizeString}\n`;
                });
                    ...
                    <div className="text-sm p-4 bg-white text-gray-800 rounded-md max-h-[50vh] overflow-auto border border-gray-300">
                        {groupedData.map(([groupName, items]) => (
                            <div key={groupName} className="mb-6">
                                ...
                                {items.map((order) => {
                                    // Calculate valid colors for display
                                    const validColors = Object.keys(order.quantities).sort().filter(color => {
                                        const sizeString = formatSizeQuantities(order.quantities[color]);
                                        return sizeString !== '';
                                    });
                                    ...
                                    return (
                                        <div key={order.id} className="ml-4 mb-4">
                                            <h3 className="text-lg font-semibold text-gray-700">
                                                ...
                                                {(order.cnpj !== 'Ambos Unidos' && order.cnpj !== 'Ambos' && order.cnpj !== 'Todos') && <span className="text-sm font-normal text-gray-500 ml-2">({order.cnpj})</span>}
                                            </h3>
                                            ...
                                            <p className="font-bold text-sm mt-2 text-blue-600">
                                                {`TOTAL: ${order.totals.totalGeral}${order.store === 'GUSHI' && order.totals.totalEspeciais ? ` (Especial: ${order.totals.totalEspeciais})` : ''}`}
                                            </p>
                                        </div>
                                    )
                                })}
        ...
        <div className={`p-4 rounded-b-xl border-x border-b dark:border-gray-700 transition-all ${isSplitting ? 'bg-purple-50 dark:bg-purple-900/20' : ''}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                                ...
                                return (
                                    <tr key={color} className="border-t border-gray-200 dark:border-gray-700">
                                        ...
                                        {order.sizes.map(size => {
                                            ...
                                            return (
                                                <td key={`${color}-${size}`} className={`p-0 text-center align-middle relative group/cell ${cellClass}`}>
                                                    {isSplitting ? (
                                                        <div className="relative h-full w-full flex items-center justify-center">
                                                            <input
                                                                ...
                                                                className="w-full h-full text-center rounded-sm border-none focus:ring-2 focus:ring-primary-500 font-bold bg-transparent placeholder-gray-400 no-spinners"
                                                            />
                                                            {(editedQty !== 0 && originalQty > 0) && (
                                                                <button
                                                                    ...
                                                                    title="Zerar item (mover tudo para faltante)"
                                                                >
                ...
                <div className="space-y-3 lg:col-span-3">
                    ...
                    {/* Pedidos Originais - Collapsible Section (like Montar screen) */}
                    <details
                        className="mt-4 group border dark:border-gray-700 rounded-lg"
                        open={true}
                    >
                        <summary className="flex justify-between items-center p-3 bg-gray-100 dark:bg-gray-700/50 rounded-t-lg cursor-pointer list-none hover:bg-gray-200 dark:hover:bg-gray-700/70 transition-colors">
                            <h3 className="text-md font-medium text-gray-700 dark:text-gray-300">📋 Pedidos Originais ({relatedRows.length})</h3>
                        ...
                        <div className="p-4 max-h-96 overflow-y-auto bg-white dark:bg-gray-800/30">
                            <RelatedOrdersTable
                                rows={relatedRows}
                                headers={headers}
...
export const EnviarPedido: React.FC<EnviarPedidoProps> = ({
    ...
    const handleSelectAll = (ordersToSelect: MergedSavedOrder[]) => {
        ...
        if (allCurrentlySelectedOnPage) {
            ...
            // Select all
            onSelectionChange(new Set([...selectedOrders, ...allVisibleIds]));
        }
    };
        ...
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {hasMultipleCnpjs && !showArchived && (
                        <div>
                            ...
                            <select id="cnpj-filter" value={cnpjFilterMode} onChange={e => onCnpjFilterChange(e.target.value as any)} className="w-full sm:w-auto pl-3 pr-10 py-2 text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-primary-500 focus:border-primary-500 rounded-md">
                                <option value="AMBOS_SEPARADOS">Ambos (Separados)</option>
                                <option value="AMBOS_UNIDOS">Ambos (Unidos)</option>
                                <option value="MM">Apenas MM</option>
                                <option value="MVF">Apenas MVF</option>
            ...
            <div className="space-y-4">
                {displayedOrders.map(order => {
                    ...
                    const personalizedCount = originalSourceRows.filter(r => isPersonalizado(String(r[skuHeader!] || ''))).length;
                                ...
                                <div className="flex-shrink-0 flex flex-wrap items-center gap-2 mt-2 sm:mt-0 justify-end">
                                    {showArchived ? (
                                        ...
                                        <>
                                            ...
                                            {personalizedCount > 0 && (
                                                <span className="px-3 py-1 text-sm font-bold rounded-md border uppercase shadow-sm whitespace-nowrap bg-pink-100 text-pink-700 border-pink-200 dark:bg-pink-900/50 dark:text-pink-300 dark:border-pink-800">
                                                    PERSONALIZADO {personalizedCount}
                                                </span>
                                            )}
