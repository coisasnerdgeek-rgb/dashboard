...
interface DashboardProps {
    metrics: {
        ...
        dailyEstampas: Record<string, { fila: number; impresso: number; aprovado: number; emAprovacao: number }>;
...
}> = ({ datasets, labels, title, formatValue, delay = 0, legend }) => {
    const [isAnimated, setIsAnimated] = React.useState(false);

    React.useEffect(() => {
        const timer = setTimeout(() => setIsAnimated(true), 100 + delay);
        ...
        <div className="w-full h-full flex flex-col">
                ...
                <div className="flex flex-col">
                    ...
                    {legend && (
                        <div className="flex gap-3 mt-1">
                            {legend.map((l, i) => (
                                <div key={i} className="text-xs font-medium flex items-center gap-1">
                                    ...
                                    <span className={l.value.startsWith('+') ? 'text-green-500' : l.value.startsWith('-') ? 'text-red-500' : 'text-gray-500'}>
                                        {l.value}
                                    </span>
                                </div>
                            ))}
            ...
            <div className="relative flex-grow h-48 group">
                <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="w-full h-full overflow-visible">
                    <defs>
                        {datasets.map((d, i) => {
                            const uniqueId = `grad-${title.replace(/[^a-zA-Z0-9]/g, '')}-${i}`;
                            return (
                                <linearGradient key={uniqueId} id={uniqueId} x1="0%" y1="0%" x2="0%" y2="100%">
                    ...
                    {datasets.map((dataset, idx) => {
                        ...
                        const fillPath = (
                            <polygon
                                ...
                                fill={`url(#grad-${title.replace(/[^a-zA-Z0-9]/g, '')}-${idx})`}
                                        ...
                                        <g key={i} className="group/point">
                                            ...
                                            >
                                                <div className="flex justify-center">
                                                    <div
                                                        className="px-2 py-1 text-white text-xs font-bold rounded shadow-lg whitespace-nowrap backdrop-blur-sm"
                                                        style={{ backgroundColor: `${dataset.color}E6` }}
                                                    >
                                                        {formattedValue}
                                                    </div>
                                                </div>
                                            ...
                                            {/* Circle point */}
                                            {/* Circle point - HIDDEN as per request */}
                                            <circle
                                                cx={x}
                                                cy={y}
                                                r="0"
                ...
                <div className="absolute bottom-[-24px] left-0 w-full h-6 text-[10px] font-medium text-gray-400 pointer-events-none">
                    {labels.map((l, i) => {
                        const leftPerc = labels.length === 1 ? 50 : (i / (labels.length - 1)) * 100;
                        return (
                            <span
                                key={i}
...
const Dashboard: React.FC<DashboardProps> = ({ metrics, setCurrentView, fazerArteCount }) => {
    ...
    React.useEffect(() => {
        if (dateRange && (dateRange.start || dateRange.end)) {
            const formatDate = (d: Date | string | null) => {
                if (!d) return '';
                if (typeof d === 'string') {
                    // If it's already an ISO string or similar, try to extract YYYY-MM-DD
                    if (d.includes('T')) return d.split('T')[0];
                    return d; // Assume it's already properly formatted if string
                }
    ...
    const salesChartData = React.useMemo(() => {
        ...
        return {
            ...
            legend: [{ label: 'vs Anterior', value: calculateChange(currentTotal, prevTotal), color: '#9ca3af' }]
        };
    ...
    const capinhasChartData = React.useMemo(() => {
        ...
        const prevDataPerso = isMounted ? prevDates.map(d => dailyCapinhas[d]?.perso || 0) : new Array(dates.length).fill(0);
        const prevDataTransp = isMounted ? prevDates.map(d => dailyCapinhas[d]?.transp || 0) : new Array(dates.length).fill(0);

        const totalPerso = dataPerso.reduce((a, b) => a + b, 0);
        const totalTransp = dataTransp.reduce((a, b) => a + b, 0);
        const totalPrevPerso = prevDataPerso.reduce((a, b) => a + b, 0);
        const totalPrevTransp = prevDataTransp.reduce((a, b) => a + b, 0);
        ...
        return {
            ...
            legend: [
                { label: 'Perso', value: calculateChange(totalPerso, totalPrevPerso), color: '#c084fc' },
                { label: 'Transp', value: calculateChange(totalTransp, totalPrevTransp), color: '#2dd4bf' }
            ]
        };
    }, [dailyCapinhas, dates, prevDates, chartLabels, isMounted]);
    ...
    const estampasChartData = React.useMemo(() => {
        ...
        const dataAprovado = isMounted ? dates.map(d => dailyEstampas[d]?.aprovado || 0) : new Array(dates.length).fill(0);
        const dataEmAprovacao = isMounted ? dates.map(d => dailyEstampas[d]?.emAprovacao || 0) : new Array(dates.length).fill(0);

        const prevDataFila = isMounted ? prevDates.map(d => dailyEstampas[d]?.fila || 0) : new Array(dates.length).fill(0);
        const totalFila = dataFila.reduce((a, b) => a + b, 0);
        const totalPrevFila = prevDataFila.reduce((a, b) => a + b, 0);

        return {
            labels: chartLabels,
            datasets: [
                { label: 'Fila', data: dataFila, color: '#f97316' },
                { label: 'Aprovado', data: dataAprovado, color: '#3b82f6' },
                { label: 'Em Aprovação', data: dataEmAprovacao, color: '#a855f7' },
                { label: 'Impresso', data: dataImpresso, color: '#22c55e' }
            ],
            legend: [
                { label: 'Fila', value: calculateChange(totalFila, totalPrevFila), color: '#f97316' }
            ]
        };
    }, [dailyEstampas, dates, prevDates, chartLabels, isMounted]);
        ...
        <div className="animate-slide-in space-y-8 pt-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col hover:shadow-lg transition-shadow duration-300 animate-slide-in" style={{ animationDelay: '700ms' }}>
                    <div className="flex-grow flex items-end pb-4">
                        <MultiLineChart datasets={capinhasChartData.datasets} labels={capinhasChartData.labels} title="Tendência de Capinhas" delay={900} legend={capinhasChartData.legend} />
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col hover:shadow-lg transition-shadow duration-300 animate-slide-in" style={{ animationDelay: '800ms' }}>
                    <div className="flex-grow flex items-end pb-4">
                        <MultiLineChart datasets={estampasChartData.datasets} labels={estampasChartData.labels} title="Fluxo de Estampas" delay={1200} legend={estampasChartData.legend} />
                    </div>
                </div>
