...
interface HeaderProps {
    ...
    onUpdateDelayRules: (newRules: DelayRules) => void;
...
const RulesEditor: React.FC<RulesEditorProps> = ({ delayRules, uniqueStoresForRules, onSave, showModal }) => {
    ...
    const filteredStores = React.useMemo(() => {
        const allStores = ['default', 'Personalizados', ...uniqueStoresForRules];
        if (activeCnpjFilter === 'todos') return allStores;

        return allStores.filter(store => {
            if (store === 'default' || store === 'Personalizados') return true;
...
// Define Icons Map matches Sidebar
const VIEW_ICONS: Record<string, React.ReactNode> = {
    ...
    pedidos: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />,
    'montar-pedido': <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />,
    'enviar-pedido': <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />,
    verificacao: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />,
    atrasados: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />,
    estampas: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" />,
    capinhas: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />,
    separacao: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />,
    pagamento: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />,
    precos: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />,
    sku: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />,
    imagem: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />,
    data: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />,
    ...
    metricas: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />,
    taxas: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />,
    default: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
};
...
export const Header: React.FC<HeaderProps> = ({
    ...
    const { pendingCount: pendingQueueCount, refreshCount: refreshQueueCount } = useQueueCount();
    ...
    // Ref to track syncing state inside event listener without re-subscribing
    ...
    // Sound Effect: Mario Coin
    // Sound Effect: Mario Coin (MP3)
    ...
    // Realtime Auto-Process Listener
    React.useEffect(() => {
        console.log("🔌 Inicializando ouvinte Realtime para fila...");

        const channel = supabase
            .channel('queue-watcher')
            .on(
                'postgres_changes',
                { event: 'INSERT', schema: 'public', table: 'webhook_retry_queue' },
                (payload) => {
                    console.log('⚡ Realtime: Novo item na fila!', payload);

                    // Prevent triggering if already syncing
                    if (isSyncingRef.current) {
                        console.log('   Visualização: Já sincronizando, ignorando trigger.');
                        return;
                    }

                    // Debounce to batch rapid inserts (wait 2s after last insert)
                    if (debounceRef.current) clearTimeout(debounceRef.current);

                    debounceRef.current = setTimeout(() => {
                        console.log('🚀 Realtime: Disparando processamento automático!');
                        playNotificationSound(); // Play Sound!
                        runProcessLoop('⚡ Auto-Processando...', 0);
                    }, 2000);
                }
            )
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') console.log('✅ Realtime: Conectado à fila.');
            });
    ...
    // Auto-Process Logic: Trigger when queue has items (Initial Load or Polling Update)
    React.useEffect(() => {
        // Condition: Has pending items AND not currently syncing AND no active debounce
        if (pendingQueueCount > 0 && pendingQueueCount < 50 && !isSyncingRef.current && !debounceRef.current) {
            console.log(`🔄 Auto-Process: Detectado ${pendingQueueCount} itens pendentes. Iniciando...`);

            debounceRef.current = setTimeout(() => {
                runProcessLoop('⚡ Auto-Processando (Fila Detectada)...', 0);
            }, 2000);
        }
    }, [pendingQueueCount]); // Runs when count updates (every 30s or manual refresh)
    ...
    const runProcessLoop = async (initialMessage: string = 'Processando pedidos...', customInitialPending?: number, customCompleted: number = 0, customFailed: number = 0) => {
        ...
        try {
            ...
            while (hasMore) {
                ...
                // Silently skip if API doesn't exist (404)
            ...
            // Silently ignore errors from non-existent API
            if (error.message && error.message.includes('JSON')) return;
    ...
    const handleSyncTiny = async (days: number = 30) => {
        setIsSyncing(true);
        setSyncProgress(prev => ({
            ...prev,
            isVisible: true,
            status: 'searching',
            message: `Buscando novos pedidos no Tiny (últimos ${days} dias)...`,
            percentage: 0,
            totalProcessed: 0,
            totalPending: 0,
            totalFailed: 0
        ...
        try {
            const response = await fetch('/api/sync-tiny', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ daysBack: days, forceReprocess: false })
            });
            const data = await response.json();
    ...
    const handleProcessQueue = async () => {
        ...
        try {
            ...
            const syncResponse = await fetch('/api/sync-tiny', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                // Lite discovery: Only check last 3 days, do NOT force reprocess old items
                body: JSON.stringify({ daysBack: 3, forceReprocess: false })
            });
                ...
                <div className="flex items-center justify-between h-16 gap-4">
                    <div className="flex items-center space-x-3 flex-shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                            {VIEW_ICONS[currentView] || VIEW_ICONS['default']}
                        </svg>
                        ...
                        {isDataLoaded && (
                            <div className="hidden lg:flex items-center gap-2 sm:gap-3 flex-grow min-w-0 justify-end">
                                ...
                                <button
                                    ...
                                    className={`
                                        flex items-center justify-center h-9 w-9 rounded-lg border transition-all focus:outline-none focus:ring-2 focus:ring-offset-2
                                        ${isRefetching
                                            ? 'bg-blue-50 border-blue-200 text-blue-600 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400 cursor-wait'
                                            : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700 hover:border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-750 dark:hover:text-gray-200'
                                        }
                                    `}
                                    title="Atualizar dados agora"
                                >
                                    <svg className={`h-5 w-5 ${isRefetching ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                                ...
                                <div className="relative flex-shrink-0" ref={syncMenuRef}>
                                    ...
                                    {showSyncMenu && (
                                        <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 py-2">
                                            <div className="px-4 py-2 text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 tracking-widest border-b dark:border-gray-700 mb-1">Sincronizar Pedidos</div>
                                            {/* Safe Sync Option (7 Days) */}
                                            <button
                                                onClick={() => { handleSyncTiny(7); setShowSyncMenu(false); }}
                                                disabled={isSyncing}
                                                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors disabled:opacity-50 disabled:cursor-wait"
                                            >
                                                ...
                                                <div className="flex-1 text-left">
                                                    <div className="text-sm font-semibold text-gray-900 dark:text-white">Tiny (7 dias)</div>
                                                    <div className="text-[10px] text-gray-500 dark:text-gray-400">Clique para forçar atualização</div>
                                                </div>
                                            </button>
                                            <button
                                            ...
                                            {/* NEW: Manual Status Update Button (Cron equivalent) */}
                                            <button
                                                onClick={() => {
                                                    runProcessLoop('Atualizando status de pedidos...');
                                                    setShowSyncMenu(false);
                                                }}
                                                disabled={isSyncing}
                                                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors disabled:opacity-50 disabled:cursor-wait border-t border-gray-100 dark:border-gray-700"
                                            >
                                                <div className="flex items-center justify-center h-8 w-8 rounded-lg bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-300">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${isSyncing ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                    </svg>
                                                </div>
                                                <div className="flex-1 text-left">
                                                    <div className="text-sm font-semibold text-gray-900 dark:text-white">Atualizar Status</div>
                                                    <div className="text-xs text-gray-500 dark:text-gray-400">
                                                        Processa fila sem buscar novos
                                                    </div>
                                                </div>
                                            </button>
                                            ...
                                            <button
                                                onClick={() => { if (onLoadMoreHistory) onLoadMoreHistory(); setShowSyncMenu(false); }}
                                                disabled={isSyncing || isHistoryLoaded}
                                                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors disabled:opacity-50"
                                            >
                                                <div className={`flex items-center justify-center h-8 w-8 rounded-lg ${isHistoryLoaded ? 'bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-300' : 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-300'}`}>
                                                ...
                                                <div className="flex-1 text-left">
                                                    <div className="text-sm font-semibold text-gray-900 dark:text-white">
                                                        {isHistoryLoaded ? 'Histórico Completo' : 'Carregar Histórico'}
                                                    </div>
                                                    <div className="text-xs text-gray-500 dark:text-gray-400">
                                                        {isHistoryLoaded ? 'Todos os dados carregados' : 'Buscando pedidos antigos...'}
                                                    </div>
                                                </div>
                                            </button>
                                        </div>
