        ...
        >
            ...
            <div className="flex-grow p-4 flex flex-col justify-between">
                <div> {/* Top part */}
                    ...
                    {!isAvulso && <p className="text-base font-semibold text-gray-700 dark:text-gray-200 mt-2 truncate" title={String(originalRow[nomeHeader] ?? '')}>{String(originalRow[nomeHeader] ?? 'N/A')}</p>}
                    ...
                    <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700"> {/* SKU Info */}
                        {editedData ? (
                            <ul className="space-y-2 text-xs">
                                {editedData.map((edit, index) => (
                                    <li key={index} className="p-2 bg-yellow-50 dark:bg-yellow-900/30 rounded-md">
                                        <p className="font-semibold text-yellow-800 dark:text-yellow-200 truncate">{transformSku(edit.sku)}</p>
                            ...
                            <div className="text-sm p-2 bg-gray-50 dark:bg-gray-900/50 rounded-md">
                                <p className="font-semibold text-gray-800 dark:text-gray-200 truncate">{transformSku(String(originalRow[skuHeader] ?? ''))}</p>
...
export const Atrasados: React.FC<AtrasadosProps> = ({ allRows, backorderedItems, resolvedItems, onAddToBackorder, onResolveBackorder, onUnresolveBackorder, onEditBackorder, onDeleteResolvedItem, onDeleteBackorder, idVendaHeader, skuHeader, quantidadeHeader, nomeHeader, dataHeader, allSkuProductNames, masterData, showModal, imageMappings }) => {
    ...
    const lateOrders = React.useMemo(() => {
        ...
        const excludedStatuses = ['enviar', 'enviado', 'em aberto', 'entregue'];
        ...
        return allRows.filter(row => {
            ...
            // Late if more than 1 business day (2 or more)
            ...
            // Check if it's clothing (exclude capinhas/estampas)
    ...
    const filteredBackorderedItems = React.useMemo(() => {
        ...
        return items;
    }, [backorderedItems, globalSearchTerm, activeTab, nomeHeader, idVendaHeader, skuHeader]);
    ...
    return (
        <div>
            ...
            {/* NEW: Auto-Detected Late Orders Section */}
            {lateOrders.length > 0 && (
                <div className="mb-8 bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-400 dark:border-yellow-600 rounded-xl p-6">
                    ...
                    <div className="space-y-2 max-h-[460px] overflow-y-auto custom-scrollbar pr-1">
                        {lateOrders.map(row => {
                            ...
                            return (
                                <div key={row._uniqueId} className="flex items-center gap-3 p-3 bg-white dark:bg-gray-800/80 rounded-lg border border-yellow-300 dark:border-yellow-700 shadow-sm hover:border-yellow-400 dark:hover:border-yellow-600 transition-colors">
                                    {/* Product Image */}
            ...
            {backorderedItems.length === 0 && (
                <div className="text-center py-16 px-6 bg-gray-50 dark:bg-gray-800/30 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl">
